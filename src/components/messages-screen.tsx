...
import { MessageCircle, Search, Heart, Shield, Sparkles, Bot } from 'lucide-react';
...
export function MessagesScreen({ session, onNavigateToDiscover, onNavigateToAIChat }: MessagesScreenProps) {
  ...
  return (
    <div className="pb-24 space-y-4">
      ...
      <div className="flex items-center gap-2 pt-2 pb-2">
        ...
        <h2 className="text-xl font-bold" style={{ fontFamily: 'Outfit, sans-serif', color: 'var(--sanjog-text-primary)' }}>Messages</h2>
        {!loading && <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-white" style={{ border: '1px solid var(--sanjog-glass-border-bright)', color: 'var(--sanjog-text-secondary)' }}>{conversations.length}</span>}
      </div>

      {/* AI Coach Pinned Message */}
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
        className="p-3.5 rounded-2xl cursor-pointer shadow-sm relative overflow-hidden group bg-white border border-gray-100"
        onClick={onNavigateToAIChat}>
        <div className="absolute -right-4 -top-4 w-16 h-16 rounded-full blur-2xl opacity-10 transition-opacity group-hover:opacity-20" style={{ background: '#d946ef' }} />
        <div className="flex items-center gap-3 relative z-10">
          <div className="w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: 'linear-gradient(135deg, #d946ef, #8b5cf6)' }}>
            <Bot className="w-5 h-5 text-white" />
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between mb-0.5">
              <span className="font-semibold text-[15px] bg-clip-text text-transparent" style={{ backgroundImage: 'linear-gradient(135deg, #d946ef, #8b5cf6)', fontFamily: 'Outfit, sans-serif' }}>Sanjog AI Coach</span>
              <span className="text-xs font-semibold text-purple-600 bg-purple-50 px-2 py-0.5 rounded-full">Active</span>
            </div>
            <p className="text-sm truncate font-medium text-gray-700">Chat with your AI Wingman to unlock Discover matches!</p>
          </div>
        </div>
      </motion.div>
      ...
      <div className="relative">
        <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4" style={{ color: 'var(--sanjog-text-secondary)' }} />
        <input
          placeholder="Search conversations…"
          value={searchQuery}
          onChange={e => setSearchQuery(e.target.value)}
          className="w-full pl-10 pr-4 py-2.5 rounded-xl text-sm outline-none bg-gray-50 border border-gray-200"
          style={{ color: 'var(--sanjog-text-primary)' }}
        />
      </div>
      ...
      <div className="flex items-center gap-2 px-3 py-2 rounded-xl" style={{ background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.2)' }}>
        ...
        <span className="text-xs font-medium" style={{ color: '#15803d' }}>End-to-end encrypted</span>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-8 text-center space-y-4">
          <div className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto" style={{ background: 'rgba(217,70,239,0.1)', border: '1px solid rgba(217,70,239,0.2)' }}>
            <MessageCircle className="w-8 h-8" style={{ color: '#d946ef' }} />
          </div>
          <div>
            <h3 className="font-bold text-lg mb-1 text-gray-900" style={{ fontFamily: 'Outfit, sans-serif' }}>{searchQuery ? 'No results' : 'No messages yet!'}</h3>
            <p className="text-sm text-gray-500">{searchQuery ? 'Try a different name.' : 'Like profiles to start conversations!'}</p>
          </div>
          {!searchQuery && (
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="relative flex-shrink-0">
                  <div className="w-14 h-14 rounded-full overflow-hidden flex items-center justify-center text-lg font-bold"
                    style={{ background: '#F3F4F6', color: '#4B5563', border: '1.5px solid rgba(244,63,94,0.30)' }}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-0.5">
                    <span className="font-semibold text-[15px] text-gray-900" style={{ fontFamily: 'Outfit, sans-serif' }}>{conv.otherParticipant?.name || 'Unknown'}</span>
                    <span className="text-xs text-gray-400 font-medium">
                      {conv.lastMessage ? timeAgo(conv.lastMessage.timestamp) : ''}
                    </span>
                  </div>
                  <p className="text-sm truncate text-gray-500 font-medium">
