...
app.get("/make-server-4644327c/health", (c) => {
  return c.json({
    status: 'ok',
  ...
  try {
    ...
    try {
      ...
      if (uploadError) {
        ...
        // Fall back to storing as data URL if storage fails
        const arrayBuffer = await file.arrayBuffer();
        const base64 = btoa(String.fromCharCode(...new Uint8Array(arrayBuffer)));
        const dataUrl = `data:${file.type};base64,${base64}`;

        // Store data URL directly in user profile
        const userProfile = await kv.get(`user:${user.id}`);
        if (userProfile) {
          const updatedPhotos = [...(userProfile.photos || []), dataUrl];
          const updatedProfile = {
            ...userProfile,
...
Deno.serve(async (req) => {
  ...
  // Supabase mounts at /functions/v1/make-server-4644327c but routes expect /make-server-4644327c
